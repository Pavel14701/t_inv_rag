...
def test_indicator_with_params_serialization() -> None:
    ...
    node = IndicatorWithParams(
        indicator="macd", params=params_multi, attributes=["line", "signal"]
    )
    data = node.to_dict()
    reconstructed = IndicatorWithParams.from_dict(data)
...
def test_comparison_serialization() -> None:
    ...
    for op in operators:
        node = Comparison(
            operator=op, left=Number(value=5.0), right=Number(value=3.0)
        )
        data = node.to_dict()
...
def test_logical_binop_serialization() -> None:
    ...
    for op in ("and", "or"):
        node = LogicalBinOp(
            operator=op, left=Number(value=1.0), right=Number(value=0.0)
        )
        data = node.to_dict()
...
def test_let_serialization() -> None:
    ...
    inner_let = Let(
        ...
        body=Comparison(
            operator=">", left=Var(name="y"), right=Number(value=10.0)
        ),
    )
...
def test_historical_access_serialization() -> None:
    ...
    for offset in (0, 1, 5):
        node = HistoricalAccess(
            expr=IndicatorAccess(indicator="close"), offset=offset
        )
        data = node.to_dict()
    ...
    indicator_with_params = IndicatorWithParams(
        indicator="rsi", params={"period": Number(value=14.0)}
    )
...
def test_from_dict_factory_all_types() -> None:
    ...
    # LogicalNot
    data = {"type": "LogicalNot", "operand": {"type": "Number", "value": 1.0}}
    node = from_dict(data)
    ...
    assert node.right.value == 3.0
    # UnaryMinus
    data = {"type": "UnaryMinus", "operand": {"type": "Number", "value": 5.0}}
    node = from_dict(data)
...
def test_round_trip_with_empty_containers() -> None:
    ...
    node_with_params = IndicatorWithParams(
        indicator="rsi", params={}, attributes=[]
    )
    data = node_with_params.to_dict()
