"""Вычислительные бэкенды: CUDA/CPU обучение, DX12 («vulkan») инференс.

Политика (TZ-06 п.11):
- обучение — только CUDA или CPU; не-CUDA обучение отброшено;
- инференс — torch (cuda/cpu) либо ONNX Runtime с DirectML EP
  (DX12: работает на AMD/Intel/NVIDIA; 'vulkan' — алиас этого пути,
  т.к. чистый Vulkan-EP в ONNX Runtime пока экспериментален);
- GGUF/llama.cpp отвергнуты: кастомная архитектура EntryExitTransformer
  не поддерживается конвертерами GGUF.

DirectML-трасса требует optional-группу зависимостей ``gpu``
(``uv sync --extra gpu``) и доступна только на Windows/DX12.
"""
...
def resolve_train_device(cfg: ComputeConfig) -> DeviceHandle:
    ...
    if backend not in _TRAIN_BACKENDS:
        raise ValueError(
            ...
            f'got {backend!r}; non-CUDA training is not supported (TZ-06 п.11)'
        )
    if backend == 'cpu':
        import torch
