...
from ..io import save_labels_parquet
...
def _make_dataset(num_bars: int, seq_len: int) -> TradingDataset:
    ...
    return TradingDataset(
        data, [], action, outcome, seq_len=seq_len,
        price_feats=5, ind_feats=3, sig_feats=2, tp_sl_feats=2,
    )
...
def test_rollback_labels_missing_backup_raises(tmp_path):
    ...
    path = tmp_path / 'labels.parquet'
    save_labels_parquet(pl.DataFrame({
        'action': [-100], 'outcome': [2.0],
    }), path)
