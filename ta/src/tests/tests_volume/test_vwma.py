...
from ...volume.vwma import vwma_numpy, vwma_ind, vwma_polars
...
def test_vwma_negative_offset(
    ...
    base = vwma_numpy(close, volume, length=10, use_talib=False)
    shifted = vwma_numpy(
        close, volume, length=10, offset=-3, use_talib=False
    )
    assert np.isnan(shifted[-3:]).all()
    assert_allclose(shifted[:-3], base[3:], rtol=1e-12, equal_nan=True)
...
def test_vwma_fillna(
    ...
    close, volume = _prices_and_volume(prices_random_walk)
    result = vwma_numpy(
        close, volume, length=10, fillna=0.0, use_talib=False
    )
...
def test_vwma_nan_policy_ignore() -> None:
    ...
    result = vwma_numpy(close, volume, length=5, nan_policy='ignore',
                        use_talib=False)
...
def test_vwma_nan_policy_ffill() -> None:
    ...
    volume = np.ones(15)
    close[5] = np.nan
    result = vwma_numpy(close, volume, length=5, nan_policy='ffill',
                        use_talib=False)
...
def test_vwma_invalid_nan_policy() -> None:
    ...
    with pytest.raises(ValueError, match='nan_policy'):
        vwma_numpy(close, np.ones(15), length=5, nan_policy='drop',
                   use_talib=False)
...
def test_vwma_inf_behaves_like_nan() -> None:
    ...
    result = vwma_numpy(close_with_inf, volume, length=5,
...
def test_vwma_backend_nan_parity() -> None:
    ...
    numba_res = vwma_numpy(close, volume, length=5, nan_policy='ignore',
                           use_talib=False)
    talib_res = vwma_numpy(close, volume, length=5, nan_policy='ignore',
                           use_talib=True)
...
def test_vwma_polars_basic(df_random_walk: pl.DataFrame) -> None:
    ...
    result_df = vwma_polars(df, length=14, output_col='VWMA',
                            use_talib=False)
    assert 'VWMA' in result_df.columns
    assert result_df['VWMA'].dtype == pl.Float64
    assert len(result_df) == len(df)
    expected = _vwma_reference(close, volume, 14)
    assert_allclose(result_df['VWMA'].to_numpy(), expected, rtol=1e-10,
                    equal_nan=True)
...
def test_vwma_polars_offset_fillna(df_random_walk: pl.DataFrame) -> None:
    ...
    volume = np.linspace(1.0, 3.0, len(close))
    df = df_random_walk.with_columns([pl.Series('volume', volume)])
    result_df = vwma_polars(df, length=10, offset=2, fillna=0.0,
                            output_col='VWMA', use_talib=False)
    expected = vwma_numpy(close, volume, length=10, offset=2, fillna=0.0,
                          use_talib=False)
    assert_allclose(result_df['VWMA'].to_numpy(), expected, rtol=1e-12,
                    equal_nan=True)
...
def test_vwma_extreme_values(prices_extreme) -> None:
    ...
    result = vwma_numpy(prices_extreme, volume, length=5,
                        nan_policy='ignore', use_talib=False)
