...
from ...momentum.stoch import stoch_ind, stoch_numpy, stoch_polars
...
def test_stoch_matches_talib(ohlc) -> None:
    ...
    expected_k, expected_d = talib.STOCH(
        high, low, close,
        fastk_period=14, slowk_period=3, slowk_matype=talib.MA_Type.SMA,
...
def test_stoch_native_matches_talib(ohlc) -> None:
    ...
    talib_k, talib_d = talib.STOCH(
        high, low, close,
        fastk_period=10, slowk_period=5, slowk_matype=talib.MA_Type.SMA,
        slowd_period=4, slowd_matype=talib.MA_Type.SMA,
    )
...
def test_stoch_invalid_params(kw: dict) -> None:
    with pytest.raises(ValueError):
        stoch_numpy(
            np.arange(40.0), np.arange(40.0), np.arange(40.0), **kw
        )
...
def test_stoch_empty_input() -> None:
    ...
    stoch_k, stoch_d = stoch_numpy(empty, empty, empty)
...
def test_stoch_polars(df_ohlc: pl.DataFrame) -> None:
    ...
    assert_allclose(
        result['STOCHk_14_3_3'].to_numpy(), stoch_k,
        rtol=1e-12, equal_nan=True,
    )
    assert_allclose(
        result['STOCHd_14_3_3'].to_numpy(), stoch_d,
        rtol=1e-12, equal_nan=True,
    )
