...
from ...momentum.coppock import coppock_ind, coppock_numpy, coppock_polars
...
def test_coppock_matches_formula(
    ...
    result = coppock_numpy(
        close, fast=fast, slow=slow, wma_length=wma_length, use_talib=False,
    )
    total_roc = _np(roc_ind(close, length=fast, use_talib=False)) + _np(
        roc_ind(close, length=slow, use_talib=False)
    )
    expected = _np(wma_ind(
        total_roc, length=wma_length, use_talib=False, nan_policy='ignore',
    ))
...
def test_coppock_warmup_nan_no_raise(
    ...
    with warnings.catch_warnings():
        ...
        result = coppock_numpy(
            prices_random_walk, slow=slow, wma_length=wma_length,
            use_talib=False,
        )
...
def test_coppock_non_contiguous(
    ...
    assert_allclose(
        coppock_numpy(close_nc, use_talib=False), expected,
...
def test_coppock_offset_fillna(
    ...
    slow, wma_length = 14, 10
    r0 = coppock_numpy(prices_random_walk, slow=slow,
                       wma_length=wma_length, use_talib=False)
    r2 = coppock_numpy(
        prices_random_walk, slow=slow, wma_length=wma_length,
        offset=2, fillna=0.0, use_talib=False,
    )
    assert r2[0] == 0.0 and r2[1] == 0.0
    start = slow + wma_length - 1  # first finite index of r0
    assert_allclose(
        r2[start + 2 :], r0[start:-2], rtol=1e-12, equal_nan=True,
    )
...
def test_coppock_polars_basic(df_ohlc: pl.DataFrame) -> None:
    ...
    assert_allclose(result['COPC_11_14_10'].to_numpy(), expected,
                    equal_nan=True)
