...
from ...momentum.ao import ao_ind, ao_numpy, ao_polars
...
def test_ao_matches_formula(
    ...
    median = (high + low) * 0.5
    expected = (
        _np(sma_ind(median, length=fast, use_talib=False))
        - _np(sma_ind(median, length=slow, use_talib=False))
    )
...
def test_ao_non_contiguous_and_readonly(
    ...
    assert_allclose(
        ao_numpy(
            mk_nc(high), mk_nc(low), fast=5, slow=10, use_talib=False,
        ),
        expected, rtol=1e-12, equal_nan=True,
    )
    assert_allclose(
        ao_ind(
            pl.Series(high), pl.Series(low), fast=5, slow=10,
            use_talib=False,
        ),
        expected, rtol=1e-12, equal_nan=True,
    )
...
def test_ao_offset_fillna(
    ...
    r2 = ao_numpy(
        high, low, fast=5, slow=slow, offset=2, fillna=0.0, use_talib=False,
    )
...
def test_ao_polars_basic(df_ohlc: pl.DataFrame) -> None:
    ...
    expected = ao_numpy(
        ...
        fast=5, slow=34, use_talib=False,
    )
