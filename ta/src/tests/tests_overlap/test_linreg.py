...
def _linreg_reference(close, length, mode='line', degrees=False):
    ...
    for i in range(length - 1, n):
        ...
        if mode == 'r':
            ...
            out[i] = (
                length * xy_sum - x_sum * y_sum
            ) / denom if denom != 0.0 else 0.0
            continue
...
def test_linreg_numba_matches_reference() -> None:
    ...
    for mode in MODE_LIST:
        result = linreg_numba(close, length=10, mode=mode)  # type: ignore[arg-type]  # noqa: E501
...
def test_linreg_numba_invalid_mode() -> None:
    ...
    with pytest.raises(ValueError, match='Unsupported mode'):
        linreg_numba(
            np.linspace(1.0, 50.0, 50), length=5, mode='bogus'  # type: ignore[arg-type]  # noqa: E501
        )
...
def test_linreg_polars_offset_fillna(df_random_walk: pl.DataFrame) -> None:
    ...
    assert_allclose(result['LINREG_line_5'].to_numpy(), expected,
...
def test_linreg_with_nan(prices_with_nan):
    """NaN input poisons only windows containing it; then recovers."""  # noqa: D403, E501
    length = 3
