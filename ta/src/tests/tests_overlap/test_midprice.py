...
def test_midprice_numba_core_matches_random_walk(
    ...
    length = 10
    rng = np.random.default_rng(42)
    high = prices_random_walk + np.abs(rng.normal(0, 0.5, len(prices_random_walk)))
    low = prices_random_walk - np.abs(rng.normal(0, 0.5, len(prices_random_walk)))
    result = _midprice_numba_core(high, low, length)
    expected = _midprice_reference(high, low, length)
...
def test_midprice_ind_matches_numba(
    ...
    result = midprice_ind(
        prices_random_walk, prices_random_walk, length=10, use_talib=False,
    )
    expected = midprice_numba(
        prices_random_walk, prices_random_walk, length=10,
    )
...
def test_midprice_ind_with_pl_series(
    ...
    rng = np.random.default_rng(42)
    high_s = pl.Series(
        prices_random_walk + np.abs(rng.normal(0, 0.5, len(prices_random_walk)))
    )
    low_s = pl.Series(
        prices_random_walk - np.abs(rng.normal(0, 0.5, len(prices_random_walk)))
    )
...
def test_midprice_ind_talib_backend(
    ...
    result = midprice_ind(
        prices_random_walk, prices_random_walk, length=5, use_talib=True,
    )
...
def test_midprice_polars_basic(df_random_walk: pl.DataFrame) -> None:
    ...
    expected = _midprice_reference(
        df['high'].to_numpy(), df['low'].to_numpy(), length,
    )
    mask = ~np.isnan(expected)
    assert_allclose(
...
def test_midprice_polars_custom_output_col(df_random_walk) -> None:
    ...
    result = midprice_polars(
        df, length=5, output_col='MIDPRICE', use_talib=False,
    )
...
def test_midprice_polars_custom_hl_cols(
    ...
    result = midprice_polars(
        df, high_col='h', low_col='l', length=10, output_col='MIDPRICE',
        use_talib=False,
    )
    expected = _midprice_reference(
        df['h'].to_numpy(), df['l'].to_numpy(), 10,
    )
    mask = ~np.isnan(expected)
...
def test_midprice_all_nan(prices_all_nan) -> None:
    ...
    assert np.isnan(result).all()
    result_fill = midprice_numba(prices_all_nan, prices_all_nan, length=3, fillna=0.0)
...
def test_midprice_extreme_values(prices_extreme) -> None:
    ...
    result = midprice_numba(
        prices_extreme + 1.0, prices_extreme - 1.0, length=3,
    )
