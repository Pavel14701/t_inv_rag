...
def _smma_reference(close: npt.NDArray[np.float64], length: int) -> npt.NDArray[np.float64]:
...
@pytest.mark.overlap
def test_smma_numba_core_basic(prices_random_walk: npt.NDArray[np.float64]) -> None:
...
def _apply_offset_fillna_test(arr: np.ndarray, offset: int, fillna: float) -> np.ndarray:
...
@pytest.mark.overlap
def test_smma_numba_basic(prices_random_walk: npt.NDArray[np.float64]) -> None:
    close = prices_random_walk
    length = 10
...
@pytest.mark.overlap
def test_smma_numba_offset_fillna(prices_random_walk: npt.NDArray[np.float64]) -> None:
...
@pytest.mark.overlap
def test_smma_numba_negative_offset(prices_random_walk: npt.NDArray[np.float64]) -> None:
    close = prices_random_walk
    length = 10
    offset = -3
    fillna = -1.0
...
@pytest.mark.overlap
def test_smma_ind_with_pl_series(prices_random_walk: npt.NDArray[np.float64]) -> None:
...
def test_smma_polars_basic(df_random_walk: pl.DataFrame) -> None:
    length = 10
    result_df = smma_polars(df_random_walk, close_col='close', length=length, output_col='SMMA')
    ...
    assert_allclose(result_df['SMMA'].to_numpy(), expected, rtol=1e-6, equal_nan=True)
...
def test_smma_polars_offset_fillna(df_random_walk: pl.DataFrame) -> None:
    ...
    offset = 3
    fillna = 0.0
    result_df = smma_polars(df_random_walk, close_col='close', length=length, offset=offset, fillna=fillna, output_col='SMMA')
    close_arr = df_random_walk['close'].to_numpy()
    expected = smma_numba(close_arr, length=length, offset=offset, fillna=fillna)
    assert_allclose(result_df['SMMA'].to_numpy(), expected, rtol=1e-6, equal_nan=True)
...
def test_smma_polars_without_output_col(df_random_walk: pl.DataFrame) -> None:
    ...
    close_arr = df_random_walk['close'].to_numpy()
    expected = _smma_reference(close_arr, length)
    assert_allclose(result_df[expected_col].to_numpy(), expected, rtol=1e-6, equal_nan=True)
