...
def _cdl_kickingbylength_nb(
    ...
    """
    Numba‑accelerated Kicking By Length pattern.
    ...
    for i in range(1, n):
        ...
        if body1 <= 0.0 or body2 <= 0.0:
            continue
        # By length: second body longer than first
        if body2 <= body1:
            continue
        # Non-overlapping bodies (gap between bodies)
...
def cdl_kickingbylength(
    ...
    """
    Universal Kicking By Length pattern.
    ...
    if use_talib and talib_available:
        talib_out = talib.CDLKICKINGBYLENGTH(open_, high, low, close)
        result = (talib_out != 0).astype(np.float64)
        return _apply_offset_fillna(result, offset, fillna)
    mask = _cdl_kickingbylength_nb(open_, high, low, close)
    out = mask.astype(np.float64)
    return _apply_offset_fillna(out, offset, fillna)


def cdl_kickingbylength_polars(
    df: pl.DataFrame,
    open_col: str = "open",
    high_col: str = "high",
    low_col: str = "low",
    ...
    offset: int = 0,
    fillna: float | None = None,
    output_col: str = "CDL_KICKINGBYLENGTH",
) -> pl.DataFrame:
    """
    Add Kicking By Length column to Polars DataFrame.
    """
    out = cdl_kickingbylength(
