...
def _cdl_eveningstar_nb(
    ...
    """
    Optimized Evening Star pattern.

    Returns:
        -1.0 → bearish evening star
         0.0 → none
    """
    n = len(open_)
    ...
    for i in range(2, n):
        ...
        # Candle 2: small body (not doji, just small)
        body1 = c1 - o1 if c1 > o1 else o1 - c1
        if body1 > min_body_factor * r1:
            continue

        # Gap up between candle 1 and candle 2
        if l1 <= h2:
            continue
...
def cdl_eveningstar(
    ...
    """
    Evening Star pattern with strict support.
    """
