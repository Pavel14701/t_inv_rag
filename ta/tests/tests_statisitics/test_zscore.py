...
from ...statistics.zscore import zscore_numpy, zscore_ind, zscore_polars
...
def test_zscore_numpy_basic(prices_random_walk: npt.NDArray[np.float64]) -> None:
    """Test zscore_numpy with default parameters on a random walk."""
    close = prices_random_walk
    n = len(close)
    result = zscore_numpy(close, length=30, multiplier=1.0, ddof=1, use_talib=False)
...
@pytest.mark.statistics
def test_zscore_numpy_multiplier(prices_random_walk: npt.NDArray[np.float64]) -> None:
    """Test that multiplier scales the z-score correctly."""
    close = prices_random_walk
    result1 = zscore_numpy(close, length=30, multiplier=1.0, ddof=1, use_talib=False)
    result2 = zscore_numpy(close, length=30, multiplier=2.0, ddof=1, use_talib=False)
...
@pytest.mark.statistics
def test_zscore_numpy_ddof(prices_random_walk: npt.NDArray[np.float64]) -> None:
    """Test that ddof affects the standard deviation calculation."""
    close = prices_random_walk
    result0 = zscore_numpy(close, length=30, multiplier=1.0, ddof=0, use_talib=False)
    result1 = zscore_numpy(close, length=30, multiplier=1.0, ddof=1, use_talib=False)
...
@pytest.mark.statistics
def test_zscore_numpy_offset_fillna(prices_random_walk: npt.NDArray[np.float64]) -> None:
    """Test offset and fillna parameters."""
    close = prices_random_walk
    result_no_offset = zscore_numpy(close, length=30, offset=0, use_talib=False)
...
@pytest.mark.statistics
def test_zscore_ind_with_pl_series(prices_random_walk: npt.NDArray[np.float64]) -> None:
    """Test zscore_ind with Polars Series input."""
    close_s = pl.Series(prices_random_walk)
    result = zscore_ind(close_s, length=30, multiplier=1.0, ddof=1, use_talib=False)
...
def test_zscore_polars_basic(df_random_walk: pl.DataFrame) -> None:
    ...
    assert_allclose(result_df['ZSCORE'].to_numpy(), expected, rtol=1e-6, equal_nan=True)


@pytest.mark.statistics
def test_zscore_polars_default_output_col(df_random_walk: pl.DataFrame) -> None:
    """Test default output column name."""
    result_df = zscore_polars(df_random_walk, close_col='close', length=30, use_talib=False)
    expected_col = 'ZS_30'
...
@pytest.mark.statistics
def test_zscore_polars_with_offset_fillna(df_random_walk: pl.DataFrame) -> None:
    ...
    close_arr = df_random_walk['close'].to_numpy()
    expected = zscore_numpy(close_arr, length=30, offset=1, fillna=0.0, use_talib=False)
    assert_allclose(result_df['ZSCORE'].to_numpy(), expected, rtol=1e-6, equal_nan=True)
...
def test_zscore_numba_algorithms_consistency(
    ...
    close = prices_random_walk
    result_online = zscore_numpy(close, length=30, use_talib=False, algorithm='online')
    result_twopass = zscore_numpy(close, length=30, use_talib=False, algorithm='two_pass')
...
@pytest.mark.statistics
def test_zscore_talib_vs_numba(prices_random_walk: npt.NDArray[np.float64]) -> None:
    ...
    result_numba = zscore_numpy(close, length=30, use_talib=False, algorithm='two_pass')
...
def test_zscore_talib_vs_numba_online(
    ...
    close = prices_random_walk
    result_talib = zscore_numpy(close, length=30, use_talib=True)
    result_numba = zscore_numpy(close, length=30, use_talib=False, algorithm='online')
