...
from ta.src.overlap.hilo import (
    _hilo_numba,
    _hilo_numba_core,
    _hilo_talib,
    hilo_ind,
...
# -----------------------------------------------------------------------------
@pytest.mark.overlap
def test_hilo_core_against_reference(
...
@pytest.mark.overlap
def test_hilo_numba_offset_fillna(
    prices_random_walk: npt.NDArray[np.float64],
) -> None:
    np.random.seed(42)
    n = len(prices_random_walk)
...
@pytest.mark.overlap
def test_hilo_numba_short_window() -> None:
...
@pytest.mark.overlap
def test_hilo_talib_against_reference(
    prices_random_walk: npt.NDArray[np.float64],
) -> None:
    np.random.seed(42)
    n = len(prices_random_walk)
    ...
    for mamode in ("sma", "ema"):
        ...
        assert_allclose(
            hilo_tl, hilo_ref, rtol=1e-3, atol=1e-3, equal_nan=True
        )
        assert_allclose(
            long_tl, long_ref, rtol=1e-3, atol=1e-3, equal_nan=True
        )
        assert_allclose(
            short_tl, short_ref, rtol=1e-3, atol=1e-3, equal_nan=True
        )


@pytest.mark.skipif(not talib_available, reason="TA-Lib not installed")
@pytest.mark.overlap
def test_hilo_talib_offset_fillna(
    prices_random_walk: npt.NDArray[np.float64],
) -> None:
    np.random.seed(42)
    n = len(prices_random_walk)
...
# -----------------------------------------------------------------------------
@pytest.mark.overlap
def test_hilo_ind_with_pl_series(
    prices_random_walk: npt.NDArray[np.float64],
) -> None:
    np.random.seed(42)
    n = len(prices_random_walk)
...
# -----------------------------------------------------------------------------
@pytest.mark.overlap
def test_hilo_polars_basic(df_random_walk: pl.DataFrame) -> None:
    np.random.seed(42)
    ...
    assert_allclose(
        result_df["HILO_test"].to_numpy(), hilo_np, rtol=1e-6, equal_nan=True
    )
    assert_allclose(
        result_df["HILOl_test"].to_numpy(), long_np, rtol=1e-6, equal_nan=True
    )
    assert_allclose(
        result_df["HILOs_test"].to_numpy(), short_np, rtol=1e-6, equal_nan=True
    )


@pytest.mark.overlap
def test_hilo_polars_offset_fillna(df_random_walk: pl.DataFrame) -> None:
    np.random.seed(42)
    n = len(df_random_walk)
    df = df_random_walk.with_columns(
        [
...
@pytest.mark.overlap
def test_hilo_polars_ema_mode(df_random_walk: pl.DataFrame) -> None:
    np.random.seed(42)
    n = len(df_random_walk)
    df = df_random_walk.with_columns(
        [
    ...
    assert_allclose(
        result_df["HILO_ema"].to_numpy(), hilo_np, rtol=1e-6, equal_nan=True
    )
...
def test_hilo_numba_with_nan(prices_with_nan):
    """HiLo with NaN in input should propagate NaNs correctly."""
