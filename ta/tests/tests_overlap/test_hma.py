...
from ta.src.overlap.hma import hma_ind, hma_numba, hma_polars
...
def test_hma_numba_against_reference_sma(
    ...
    assert_allclose(
        result[start:], expected[start:], rtol=1e-6, equal_nan=True
    )
...
def test_hma_polars_offset_fillna(df_random_walk: pl.DataFrame) -> None:
    ...
    assert_allclose(
        result_df["HMA"].to_numpy(), expected, rtol=1e-6, equal_nan=True
    )
...
def test_hma_polars_with_nan(df_random_walk):
    ...
    result_df = hma_polars(
        df_with_nan, close_col="close", length=5, output_col="HMA"
    )
    assert "HMA" in result_df.columns
