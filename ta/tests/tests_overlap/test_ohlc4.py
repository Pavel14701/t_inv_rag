...
def test_ohlc4_against_reference(
    ...
    result = ohlc4_numpy(o, h, l, c)
    expected = _ohlc4_reference(o, h, l, c)
...
def test_ohlc4_between_low_and_high() -> None:
    ...
    o, h, l, c = _make_ohlc(close, seed=1)
    result = ohlc4_numpy(o, h, l, c)
...
def test_ohlc4_offset_fillna(
    ...
    offset = 2
    fillna = 0.0
    base = ohlc4_numpy(o, h, l, c, offset=0, fillna=None)
    expected = _apply_offset_fillna(base, offset, fillna)
    result = ohlc4_numpy(o, h, l, c, offset=offset, fillna=fillna)
    assert_allclose(result, expected, rtol=1e-12, equal_nan=True)
...
def test_ohlc4_ind_matches_numpy(
    ...
    o, h, l, c = _make_ohlc(prices_random_walk)
    result = ohlc4_ind(o, h, l, c)
    expected = ohlc4_numpy(o, h, l, c)
...
def test_ohlc4_ind_with_pl_series(
    ...
    o, h, l, c = _make_ohlc(prices_random_walk)
    result = ohlc4_ind(pl.Series(o), pl.Series(h), pl.Series(l), pl.Series(c))
    expected = ohlc4_numpy(o, h, l, c)
    assert_allclose(result, expected, rtol=1e-12)
