...
from ...overlap.zlma import _MA_FUNCS, _call_ma, zlma_ind, zlma_polars
...
def test_zlma_polars_basic(df_random_walk: pl.DataFrame) -> None:
    ...
    result = zlma_polars(
        df_random_walk, length=10,
        mamode='ema', use_talib=False
    )
    col = 'ZL_EMA_10'
...
def test_zlma_with_nan(prices_with_nan):
    """NaN in input propagates; output has no Inf escalation."""  # noqa: D403, E501
...
def test_zlma_all_nan(prices_all_nan):
    ...
    filled = zlma_ind(
        prices_all_nan, length=3, mamode='wma',
        fillna=0.0, use_talib=False
    )
