...
from ta.src.overlap.mama import (
    _mama_numba_core,
    mama_ind,
    mama_numba,
    mama_polars,
...
def test_mama_numba_offset_fillna():
    ...
    offset = 2
    fillna = 0.0
    base_mama, base_fama = mama_numba(
        close, fastlimit, slowlimit, prenan, offset=0, fillna=None
    )
    expected_mama = _apply_offset_fillna(base_mama, offset, fillna)
    expected_fama = _apply_offset_fillna(base_fama, offset, fillna)
    result_mama, result_fama = mama_numba(
        close, fastlimit, slowlimit, prenan, offset=offset, fillna=fillna
    )
...
def test_mama_numba_nan_policy_ffill():
    ...
    prenan = 3
    result_mama, result_fama = mama_numba(
        close, fastlimit, slowlimit, prenan, nan_policy="ffill"
    )
...
def test_mama_numba_nan_policy_ignore():
    ...
    prenan = 3
    result_mama, result_fama = mama_numba(
        close, fastlimit, slowlimit, prenan, nan_policy="ignore"
    )
    assert np.isnan(result_mama[:2]).all()
    assert np.isnan(result_fama[:2]).all()
    assert np.isnan(result_mama[2:]).all()
...
def test_mama_numba_nan_policy_bfill():
    ...
    prenan = 3
    result_mama, result_fama = mama_numba(
        close, fastlimit, slowlimit, prenan, nan_policy="bfill"
    )
    assert np.isnan(result_mama[:2]).all()
    assert np.isnan(result_fama[:2]).all()
    assert np.isnan(result_mama[2:]).all()
...
def test_mama_ind_uses_numba():
    ...
    slowlimit = 0.05
    prenan = 3
    result_mama, result_fama = mama_ind(
        close, fastlimit, slowlimit, prenan, use_talib=False
    )
    expected_mama, expected_fama = mama_numba(
        close, fastlimit, slowlimit, prenan
    )
    assert_allclose(result_mama, expected_mama, rtol=1e-6)
    assert_allclose(result_fama, expected_fama, rtol=1e-6)
...
def test_mama_numba_with_nan(prices_with_nan):
    ...
    prenan = 3
    result_mama, result_fama = mama_numba(
        prices_with_nan, fastlimit, slowlimit, prenan, nan_policy="ignore"
    )
    assert np.isnan(result_mama[:2]).all()
    assert np.isnan(result_fama[:2]).all()
...
def test_mama_numba_with_inf(prices_with_inf):
    ...
    prenan = 3
    result_mama, result_fama = mama_numba(
        prices_with_inf, fastlimit, slowlimit, prenan, nan_policy="ignore"
    )
    assert np.isnan(result_mama[:2]).all()
    assert np.isnan(result_fama[:2]).all()
    assert np.isnan(result_mama[5:]).all()
...
def test_mama_numba_all_nan(prices_all_nan):
    ...
    prenan = 3
    result_mama, result_fama = mama_numba(
        prices_all_nan, fastlimit, slowlimit, prenan, nan_policy="ignore"
    )
    assert np.isnan(result_mama).all()
    assert np.isnan(result_fama).all()
    result_mama_fill, result_fama_fill = mama_numba(
        prices_all_nan,
        fastlimit,
        slowlimit,
        prenan,
...
def test_mama_numba_extreme_values(prices_extreme):
    ...
    prenan = 3
    result_mama, result_fama = mama_numba(
        prices_extreme, fastlimit, slowlimit, prenan, nan_policy="ignore"
    )
