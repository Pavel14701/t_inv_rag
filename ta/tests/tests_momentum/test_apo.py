...
from ...momentum.apo import apo_ind, apo_numpy, apo_polars
...
def test_apo_matches_formula_sma(
    ...
    fast, slow = 12, 26
    result = apo_numpy(close, fast=fast, slow=slow, mamode='sma',
                       use_talib=False)
    expected = (
        _np(sma_ind(close, length=fast, use_talib=False))
        - _np(sma_ind(close, length=slow, use_talib=False))
    )
...
def test_apo_ema_mode_parity(
    ...
    expected = (
        _np(ma_mode('ema', close, length=5, offset=0, fillna=None))
...
def test_apo_offset_fillna(
    ...
    r2 = apo_numpy(
        close, fast=5, slow=slow, offset=2, fillna=0.0, use_talib=False,
    )
    assert r2[0] == 0.0 and r2[1] == 0.0
    assert_allclose(
        r2[slow + 1 :], r0[slow - 1 : -2], rtol=1e-12, equal_nan=True,
    )
...
def test_apo_polars_basic(df_ohlc: pl.DataFrame) -> None:
    ...
    expected = apo_numpy(
        df_ohlc['close'].to_numpy(), fast=12, slow=26, use_talib=False,
    )
