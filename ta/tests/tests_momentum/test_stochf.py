...
from ...momentum.stochf import stochf_ind, stochf_numpy, stochf_polars
...
def test_stochf_matches_formula(
    ...
    stoch_k, stoch_d = stochf_numpy(high, low, close, k=k, d=d,
                                    use_talib=False)
...
def test_stochf_backend_parity(
    ...
    high, low = _hl_arrays(prices_random_walk)
    k_np, d_np = stochf_numpy(high, low, prices_random_walk,
                              use_talib=False)
    k_tl, d_tl = stochf_numpy(high, low, prices_random_walk,
                              use_talib=True)
...
def test_stochf_warmup_nan(
    ...
    high, low = _hl_arrays(prices_random_walk)
    k, d = 14, 3
    stoch_k, stoch_d = stochf_numpy(high, low, prices_random_walk, k=k, d=d,
                                    use_talib=False)
...
def test_stochf_zero_range_no_warning() -> None:
    ...
    with warnings.catch_warnings():
        ...
        stoch_k, _ = stochf_numpy(high, low, close, k=14, d=3,
                                  use_talib=False)
...
def test_stochf_non_contiguous(
    ...
    assert_allclose(
        stochf_ind(pl.Series(high), pl.Series(low),
...
def test_stochf_offset_fillna(
    ...
    high, low = _hl_arrays(prices_random_walk)
    k = 14
    k0, d0 = stochf_numpy(high, low, prices_random_walk, k=k,
                          use_talib=False)
    k2, d2 = stochf_numpy(
        high, low, prices_random_walk, k=k, offset=2, fillna=0.0,
        use_talib=False,
    )
...
def test_stochf_polars_basic(df_ohlc: pl.DataFrame) -> None:
    ...
    assert_allclose(result['STOCHFk_14_3'].to_numpy(), expected_k,
                    equal_nan=True)
    assert_allclose(result['STOCHFd_14_3'].to_numpy(), expected_d,
                    equal_nan=True)
