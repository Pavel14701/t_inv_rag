...
from ta.src.candle.pf_trend import _pf_trend_nb, pf_trend, pf_trend_polars
...
@pytest.mark.parametrize(
    "prices, box_size, reversal, expected, desc", PF_TREND_TEST_CASES
)
...
def test_pf_trend_polars_df_uptrend(
    ...
    result_df = pf_trend_polars(
        df_uptrend, price_col="close", box_size=box_size, reversal=reversal
    )
...
def test_pf_trend_polars_df_random_walk(
    ...
    result_df = pf_trend_polars(
        df_random_walk, price_col="close", box_size=box_size, reversal=reversal
    )
    assert "PF_TREND" in result_df.columns
...
def test_pf_trend_polars() -> None:
    ...
    result_df = pf_trend_polars(
        df, price_col="close", box_size=2.0, reversal=3, output_col="PF_TREND"
    )
    assert "PF_TREND" in result_df.columns
    expected_values = [0.0, 1.0, 1.0, -1.0, -1.0]
    np.testing.assert_allclose(
        result_df["PF_TREND"].to_numpy(), np.array(expected_values)
    )
...
def pf_trend_python(prices, box_size, reversal):
    n = len(prices)
