...
from ta.src.volatility.true_range import (
...
def test_tr_nan_propagates_with_ignore(df_ohlc: pl.DataFrame) -> None:
    """NaN in high/low/close propagates into the affected TR bars."""
...
def test_tr_inf_replaced_with_nan(df_ohlc: pl.DataFrame) -> None:
    ...
    result = true_range_numba(high_with_inf, low, close, nan_policy="ignore")
...
def test_tr_polars_basic(df_ohlc: pl.DataFrame) -> None:
    ...
    result_df = true_range_polars(df_ohlc, output_col="TR", use_talib=False)
    ...
    assert_allclose(
        result_df["TR"].to_numpy(), expected, rtol=1e-12, equal_nan=True
    )
...
def test_tr_all_nan_prices() -> None:
    ...
    result = true_range_numba(nan_arr, nan_arr, nan_arr, nan_policy="ignore")
