...
from ta.src.volatility.atr import atr_ind, atr_numba, atr_polars, atr_talib
...
def test_atr_numba_sma_and_ema_against_reference(
    ...
    for mode in ("sma", "ema"):
        ...
        expected = _atr_reference(high, low, close, length=10, mamode=mode)
        assert_allclose(result, expected, rtol=1e-10, equal_nan=True)
...
def test_atr_inf_replaced_with_nan(df_ohlc: pl.DataFrame) -> None:
    ...
    result = atr_numba(high_with_inf, low, close, length=5, nan_policy="ffill")
