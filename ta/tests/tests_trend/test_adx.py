...
from ta.src.trend.adx import adx_ind, adx_numpy, adx_polars
...
def test_adx_tvmode_not_all_nan(ohlc) -> None:
    ...
    np.testing.assert_allclose(
        np.nanmedian(dmp_tv[-100:]),
        np.nanmedian(dmp_std[-100:]),
        rtol=0.1,
    )
...
def test_adx_talib_parity(ohlc) -> None:
    ...
    np.testing.assert_allclose(
        np.nanmedian(adx_talib[-100:]),
        np.nanmedian(adx_np[-100:]),
        atol=1e-6,
    )
